...
class html {
	...
	public function category($catid, $page = 0) {
		...
		if($create_to_html_root) {
			if($this->siteid==1) {
				$file = ZLCMS_PATH.$base_file;
			} else {
				$file = ZLCMS_PATH.substr($this->html_root,1).$base_file;
			}
			//添加到发布点队列
	...
	*/
	private function createhtml($file, $copyjs = '') {
		...
		@chmod($file,0777);
		if(!is_writable($file)) {
			$file = str_replace(ZLCMS_PATH,'',$file);
