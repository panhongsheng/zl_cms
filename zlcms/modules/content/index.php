...
class index {
    ...
    //    $_GET['dir'] = 'poster';
        ...
        $this->upload_path = pc_base::load_config('system','upload_path').'../product';
        ...
        $local = str_replace(array(PC_PATH, ZLCMS_PATH ,DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR), array('','',DIRECTORY_SEPARATOR), $filepath);




        function fetchDir($dir,$file_url = '') {
            foreach(glob($dir.'\*') as $file) {
                if(is_dir($file)) {
                   // echo $file.'<br>';
                    $file_url .= $file;
                    fetchDir($file,$file_url);
                }
            }
            return $file_url;
        }
       echo fetchDir($this->upload_path);
        //print_r(fetchDir($this->upload_path)) ;
        ...
        $result = my_scandir($this->upload_path);
        print_r($result);
        $result1 = my_scandir1($this->upload_path);
        print_r($result1);
        ?>
    ...
    foreach($list as $v) {
        ...
        <tr>
            <?php if (is_dir($v)) {
                ...
                <a rel="'.$local.'/'.$filename.'">'.$filename.'</a></td><td width="10%"><a href="javascript:;" onclick="preview(\''.$local.'/'.$filename.'\')">'.L('preview').'</a> | <a href="javascript:;" onclick="att_delete(this,\''.urlencode($filename).'\',\''.urlencode($local).'\')">'.L('delete').'</a> </td>';
            }?>
        </tr>
    <?php
    }
