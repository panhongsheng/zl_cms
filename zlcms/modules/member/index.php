...
class index extends foreground {
	...
	public function register() {
		...
		if(isset($_POST['dosubmit'])) {
				...
				if($status > 0) {
					...
					if($userid > 0) {
						...
						if($userinfo['groupid'] == 7) {
							...
							$ZLCMS_auth_key = md5(pc_base::load_config('system', 'auth_key').$this->http_user_agent);
							$ZLCMS_auth = sys_auth($userid."\t".$userinfo['password'], 'ENCODE', $ZLCMS_auth_key);
							
							param::set_cookie('auth', $ZLCMS_auth, $cookietime);
					...
					if($member_setting['enablemailcheck']) {
						...
						$ZLCMS_auth_key = md5(pc_base::load_config('system', 'auth_key'));
						$code = sys_auth($userid.'|'.SYS_TIME, 'ENCODE', $ZLCMS_auth_key);
			...
			if(!empty($_GET['verify'])) {
				...
				$ZLCMS_auth_key = md5(pc_base::load_config('system', 'auth_key'));
				$code_res = sys_auth($code, 'DECODE', $ZLCMS_auth_key);
	...
	public function login() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$ZLCMS_auth_key = md5(pc_base::load_config('system', 'auth_key').$this->http_user_agent);
			$ZLCMS_auth = sys_auth($userid."\t".$password, 'ENCODE', $ZLCMS_auth_key);
	...
	public function public_forget_password () {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$code = sys_auth($memberinfo['userid']."\t".SYS_TIME, 'ENCODE', $ZLCMS_auth_key);
			...
			$code = sys_auth($_GET['code'], 'DECODE', $ZLCMS_auth_key);
			$code = explode("\t", $code);
	...
	/**
	*通过手机修改密码
	*方式：用户发送HHPWD afei985#821008 至 1065788 ，ZLCMS进行转发到网站运营者指定的回调地址，在回调地址程序进行密码修改等操作,处理成功时给用户发条短信确认。
	*ZLCMS 以POST方式传递相关数据到回调程序中
