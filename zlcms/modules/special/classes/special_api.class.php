...
class special_api {
	...
	public function _del_special($id = 0) {
		...
		if ($info['siteid']>1) {
			if ($info['ishtml']) {
				...
				if ($info['ispage']) {
					for ($i==1; $i>0; $i++) {
						...
						if (!file_exists(ZLCMS_PATH.$file)) {
							break;
						} else {
							$queue->add_queue('del', $file, $info['siteid']);
							unlink(ZLCMS_PATH.$file);
						}
					}
				} else {
					$queue->add_queue('del', $file, $info['siteid']);
			...
			if ($info['ishtml']) {
				dir_delete(ZLCMS_PATH.pc_base::load_config('system', 'html_root').DIRECTORY_SEPARATOR.'special'.DIRECTORY_SEPARATOR.$info['filename']); //删除专题目录
			}
		}
			...
			if ($siteid>1) {
				...
				for ($i = 1; $i>0; $i++) {
					...
					if (!file_exists(ZLCMS_PATH.$file)) {
						...
						unlink(ZLCMS_PATH.$file);	//删除生成的静态文件
					}
				}
			} else {
				for ($i = 1; $i>0; $i++) {
			...
			if ($ishtml) {	
				...
				if ($siteid>1) {
					...
					for ($i = 1; $i>0; $i++) {
						...
						if (!file_exists(ZLCMS_PATH.$file[1])) {
							break;
						} else {
							$queue->add_queue('del', $file[1], $siteid); //并加入到消息队列中，便以其他站点删除文件
							unlink(ZLCMS_PATH.$file[1]);	//删除生成的静态文件
						}
					}
				} else {
					for ($i = 1; $i>0; $i++) {
