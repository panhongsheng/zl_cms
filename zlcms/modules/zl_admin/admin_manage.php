...
class admin_manage extends admin
{
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if ($this->db->insert_id()) {
                showmessage(L('operation_success'), '?m=zl_admin&c=admin_manage');
            }
        } else {
    ...
    public function public_edit_pwd()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            showmessage(L('password_edit_succ_logout'), '?m=zl_admin&c=index&a=public_logout');
        } else {
    ...
    public function public_edit_info()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            include $this->admin_tpl('zl_admin_edit_info');
        }
        ...
        if ($data = $this->db->get_one(array('userid' => $userid), '`card`,`username`')) {
            if (empty($data['card'])) {
                ...
                if ($this->db->update(array('card' => $card), array('userid' => $userid))) {
                    showmessage(L('password_card_application_success'), '?m=zl_admin&c=admin_manage&a=card&userid=' . $userid);
                } else {
    ...
    function remove_card()
    {
        ...
        if ($data = $this->db->get_one(array('userid' => $userid), '`card`,`username`,`userid`')) {
            ...
            if ($result = card::remove_card($data['card'])) {
                ...
                showmessage(L('the_binding_success'), '?m=zl_admin&c=admin_manage&a=card&userid=' . $userid);
            }
        } else {
            showmessage(L('users_were_not_found'));
        }
