...
class badword extends admin {
	...
	function init () {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=zl_admin&c=badword&a=add\', title:\''.L('badword_add').'\', width:\'450\', height:\'180\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('badword_add'));
		...
		if(isset($_POST['dosubmit'])){
				...
				$_POST['info']['badword'] = str_replace("　","",trim($_POST['badword']));
	 			if(empty($_POST['info']['badword'])) {
					showmessage(L('enter_word'),'?m=zl_admin&c=badword&a=add');
					}
				$this->db->insert($_POST['info']);
				$this->public_cache_file();//更新缓存
		 		showmessage(L('operation_success'),'?m=zl_admin&c=badword&a=add','', 'add');
		 		}else{
	...
	function listorder() {
		if(!is_array($_POST['listorders'])) return false;
			...
			showmessage(L('operation_success'),'?m=zl_admin&c=badword');
	}
	...
	function edit() {
		if(isset($_POST['dosubmit'])){
				...
				$this->public_cache_file();//更新缓存
				showmessage(L('operation_success'),'?m=zl_admin&c=badword&a=edit','', 'edit');
			}else{
				$show_validator = $show_scroll = $show_header = true;
				$info = array();
	...
	function delete() {
 		if(is_array($_POST['badid'])){
				...
				if($result){
					...
					showmessage(L("operation_failure"),'?m=zl_admin&c=badword');
				}
		}
	}
	...
	function export() {
		...
		if(!is_array($result) || empty($result)){
			showmessage('暂无敏感词设置，正在返回！','?m=zl_admin&c=badword');
		}
  		foreach($result as $s){
 			extract($s);
	...
	function import(){
		if(isset($_POST['dosubmit'])){
				$arr = $s = $str = $level_arr = '';
				$s = trim($_POST['info']);
			    if(empty($s)) showmessage(L('not_information'),'?m=zl_admin&c=badword&a=import');
	 			$arr = explode("\n",$s); 
