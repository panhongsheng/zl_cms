...
class cache_all extends admin {
	...
	public function init() {
		if (isset($_POST['dosubmit']) || isset($_GET['dosubmit'])) {
			...
			$this->cache_api = pc_base::load_app_class('cache_api', 'zl_admin');
			$m = $modules[$page];
			if ($m['mod'] && $m['function']) {
			...
			if (!empty($modules[$page])) {
				...
				showmessage(L('update').$m['name'].L('cache_file_success'), '?m=zl_admin&c=cache_all&page='.$page.'&dosubmit=1&pc_hash='.$_SESSION['pc_hash'], 0);
			} else {
