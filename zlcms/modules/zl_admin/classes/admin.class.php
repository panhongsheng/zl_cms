...
class admin {
	...
	final public function check_admin() {
		if(ROUTE_M =='zl_admin' && ROUTE_C =='index' && in_array(ROUTE_A, array('login', 'public_card'))) {
			return true;
		} else {
			$userid = param::get_cookie('userid');
			if(!isset($_SESSION['userid']) || !isset($_SESSION['roleid']) || !$_SESSION['userid'] || !$_SESSION['roleid'] || $userid != $_SESSION['userid']) showmessage(L('admin_login'),'?m=zl_admin&c=index&a=login');
		}
	}
	...
	final public static function get_siteinfo($siteid = '') {
		...
		$sites = pc_base::load_app_class('sites', 'zl_admin');
	...
	final public function check_priv() {
		if(ROUTE_M =='zl_admin' && ROUTE_C =='index' && in_array(ROUTE_A, array('login', 'init', 'public_card'))) return true;
	...
	final private function lock_screen() {
		if(isset($_SESSION['lock_screen']) && $_SESSION['lock_screen']==1) {
			...
			showmessage(L('admin_login'),'?m=zl_admin&c=index&a=login');
		}
	}
