...
class cache_api {
	...
	public function membermodel() {
		...
		//模型缓存路径
		define('MEMBER_CACHE_MODEL_PATH',ZLCMS_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_model'.DIRECTORY_SEPARATOR.'caches_data'.DIRECTORY_SEPARATOR);
	...
	public function cache2database() {
		...
		if ($currpage>$pages) {
			...
			showmessage(L('part_cache_success'), '?m=zl_admin&c=cache_all&a=init&page='.$_GET['page'].'&currpage='.$currpage.'&pages='.$pages.'&dosubmit=1',0);
		}
	}
	...
	public function del_file() {
		$path = ZLCMS_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR;
		$files = glob($path.'*');
		...
		$path = ZLCMS_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_tpl_data'.DIRECTORY_SEPARATOR.'caches_data'.DIRECTORY_SEPARATOR;
		$files = glob($path.'*');
		if (is_array($files)) {
			foreach ($files as $f) {
				$dir = basename($f);
	...
	public function copyfrom() {
		...
		setcache('copyfrom', $infos, 'zl_admin');
		return true;
	}
	/**
	 * 同步视频模型栏目
