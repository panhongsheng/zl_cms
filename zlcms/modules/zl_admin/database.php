...
class database extends admin {
		...
		if($_GET['dosubmit']) {
			...
			if(is_array($sqlfiles)) {
				...
				foreach($sqlfiles as $id=>$sqlfile) {
					//老的数据库备份文件转换为新格式
					if(preg_match("/([ZLCMStables_|db_][0-9]{8}_[0-9a-z]{20}_)([0-9]+)\.sql/i",basename($sqlfile),$num)) {
					...
					if(preg_match("/([0-9a-z]{20}_[ZLCMStables_|db_]+[0-9]{8}_)([0-9]+)\.sql/i",basename($sqlfile),$num)) {
	...
	public function public_repair() {
		...
		if($tables && in_array($operation,array('repair','optimize'))) {
			...
			showmessage(L('operation_success'),'?m=zl_admin&c=database&a=export&pdoname='.$pdo_name);
			...
			showmessage(L('select_tbl'),'?m=zl_admin&c=database&a=export&pdoname='.$pdo_name);
		}
	}
	...
	public function delete() {
		...
		if($filenames) {
			if(is_array($filenames)) {
				...
				showmessage(L('operation_success'),'?m=zl_admin&c=database&a=import&pdoname='.$pdo_name);
			} else {
	...
	private function status($tables,$tablepre) {
		...
		return array('ZLCMStables'=>$ZLCMS, 'othertables'=>$other);
	}
	 ...
	 * @param unknown_type $tabletype 备份数据库类型 （非ZLCMS数据与ZLCMS数据）
	 */
	private function export_database($tables,$sqlcompat,$sqlcharset,$sizelimit,$action,$fileid,$random,$tableid,$startfrom,$tabletype) {
		...
		if(trim($tabledump)) {
			...
			showmessage(L('bakup_file')." $filename ".L('bakup_write_succ'), '?m=zl_admin&c=database&a=export&sizelimit='.$sizelimit.'&sqlcompat='.$sqlcompat.'&sqlcharset='.$sqlcharset.'&tableid='.$tableid.'&fileid='.$fileid.'&startfrom='.$startrow.'&random='.$random.'&dosubmit=1&tabletype='.$tabletype.'&allow='.$allow.'&pdo_select='.$this->pdo_name);
		} else {
		   ...
		   delcache('bakup_tables','commons');
		   showmessage(L('bakup_succ'),'?m=zl_admin&c=database&a=import&pdoname='.$this->pdo_name);
		}
	}
	/**
	 * 数据库恢复
	...
	private function import_database($filename) {
		if($filename && fileext($filename)=='sql') {
			...
			if(file_exists($filepath)) {
				...
				$fileid++;
				showmessage(L('bakup_data_file')." $filename ".L('load_success'),"?m=zl_admin&c=database&a=import&pdoname=".$this->pdo_name."&pre=".$pre."&fileid=".$fileid."&dosubmit=1");
			} else {
				showmessage(L('data_recover_succ'),'?m=zl_admin&c=database&a=import');
			}
		}
	}
	 ...
	 */
 	...
 	private function sql_split($sql) {
		...
		if($this->db_tablepre != "ZLCMS_") $sql = str_replace("`ZLCMS_", '`'.$this->db_tablepre, $sql);
		$sql = str_replace("\r", "\n", $sql);
		$ret = array();
		$num = 0;
