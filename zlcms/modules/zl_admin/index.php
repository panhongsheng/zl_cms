...
class index extends admin {
	...
	public function init () {
		...
		include $this->admin_tpl('zl_index');
	}
		...
		if(isset($_GET['dosubmit'])) {
			...
			if (!isset($_GET['card'])) {
				...
				if (!isset($_SESSION['card_verif']) || $_SESSION['card_verif'] != 1) {
					showmessage(L('your_password_card_is_not_validate'), '?m=zl_admin&c=index&a=public_card');
				}
...
*/
			...
			if(!$r) showmessage(L('user_not_exist'),'?m=zl_admin&c=index&a=login');
			...
			if($r['password'] != $password) {
				...
				showmessage(L('password_error',array('times'=>$times)),'?m=zl_admin&c=index&a=login',3000);
			}
			...
			if (!isset($_GET['card']) && $r['card'] && pc_base::load_config('system', 'safe_card') == 1) {
				...
				header("location:?m=zl_admin&c=index&a=public_card");
				exit;
			...
			showmessage(L('login_success'),'?m=zl_admin&c=index');
			//同步登陆vms,先检查是否启用了vms
			...
			include $this->admin_tpl('zl_login');
		}
	}
	public function public_card() {
		...
		if(!$r) showmessage(L('user_not_exist'),'?m=zl_admin&c=index&a=login');
		if (isset($_GET['dosubmit'])) {
			pc_base::load_app_class('card', 'zl_admin', 0);
			$result = card::verification($r['card'], $_POST['code'], $_POST['rand']);
			$_SESSION['card_verif'] = 1;
			header("location:?m=zl_admin&c=index&a=login&dosubmit=1&card=1");
			exit;
		}
		pc_base::load_app_class('card', 'zl_admin', 0);
	...
	public function public_logout() {
		...
		showmessage(L('logout_success').$phpsso_logout,'?m=zl_admin&c=index&a=login');
	}
	//左侧菜单
	public function public_menu_left() {
		...
		if (isset($_GET['parentid']) && $parentid = intval($_GET['parentid']) ? intval($_GET['parentid']) : 10) {
			...
			include $this->admin_tpl('zl_left');
		}
    ...
    //当前位置
    public function public_current_pos_zl() {
        $data = admin::current_pos($_GET['menuid']);
        $str = explode('>',rtrim($data,'>'));
        $zl_pos = '';
        $i = 0;
        foreach($str as $val){
            $i++;
            $first = $i == 1?'<i class="icon-home"></i>':'';
            $zl_pos.= trim($val)?'<li>'.$first.$val.'<i class="icon-angle-right"></i></li>':'';
        }
        echo $zl_pos;
        exit;
    }
	...
	public function public_ajax_add_panel() {
		...
		foreach($panelarr as $v) {
			echo "<span><a onclick='paneladdclass(this);' target='right' href='".$v['url'].'&menuid='.$v['menuid']."&pc_hash=".$_SESSION['pc_hash']."'>".L($v['name'])."</a>  <a class='panel-delete icon-remove icon-white' href='javascript:delete_panel(".$v['menuid'].");'></a></span>";
		}
		exit;
	}
	public function public_ajax_delete_panel() {
		...
		$panelarr = $this->panel_db->listinfo(array('userid'=>$_SESSION['userid']), "datetime");
		foreach($panelarr as $v) {
			echo "<span><a onclick='paneladdclass(this);' target='right' href='".$v['url']."&pc_hash=".$_SESSION['pc_hash']."'>".L($v['name'])."</a> <a class='panel-delete icon-remove icon-white' href='javascript:delete_panel(".$v['menuid'].");'></a></span>";
		}
		exit;
	}
	...
	public function public_snda_status() {
		...
		if(!strstr(pc_base::load_config('snda','snda_status'), '|')) {
			...
			$uuid = $uuid_arr['uuid'];
			$snda_check_url = "http://open.sdo.com/ZLCMS?cmsid=".$uuid."&sitedomain=".$_SERVER['SERVER_NAME'];
