...
class plugin extends admin {
	...
	public function import() {
		if(!isset($_GET['dir'])) {
			...
			if(empty($_GET['license']) && $license) {
				$submit_url = '?m=zl_admin&c=plugin&a=import&dir='.$dir.'&license=1&pc_hash='. $_SESSION['pc_hash'].'&menuid='.$_GET['menuid'];
			} else {
				$submit_url = '?m=zl_admin&c=plugin&a=install&dir='.$dir.'&pc_hash='. $_SESSION['pc_hash'].'&menuid='.$_GET['menuid'];
			}	
				$show_header = 0;
	...
	public function install() {
		...
		if($op_status) {	
			...
			showmessage(L('plugin_install_success','','plugin'),'?m=zl_admin&c=plugin');
		} else {
			showmessage(L('plugin_install_fail','','plugin'),'?m=zl_admin&c=plugin');
		}
	}	
	...
	public function listorder() {
		if(isset($_POST['dosubmit'])) {
			...
			$this->set_hook_cache();
			showmessage(L('operation_success'),'?m=zl_admin&c=plugin');
		} else {
			showmessage(L('operation_failure'),'?m=zl_admin&c=plugin');
		}
	}
	...
	public function appcenter() {
		...
		$data = file_get_contents('http://open.ZLCMS.cn/index.php?m=open&c=api&a=get_applist&s='.$s.'&p='.$p);
		$data = array_iconv(json_decode($data, true),'utf-8',CHARSET);
		
		$recommed_data = file_get_contents('http://open.ZLCMS.cn/index.php?m=open&c=api&a=get_recommed_applist&s=5&p=1');
		$recommed_data = array_iconv(json_decode($recommed_data, true),'utf-8',CHARSET);
		
		$focus_data = file_get_contents('http://open.ZLCMS.cn/index.php?m=open&c=api&a=get_app_focus&num=3');
		$focus_data = array_iconv(json_decode($focus_data, true),'utf-8',CHARSET);	
		$pages = $data['pages'];
		$pre_page = $p <= 1 ? 1 : $p - 1;
		$next_page = $p >= $pages ? $pages : $p + 1;
		$pages = '<a class="a1">'.$data['total'].L('plugin_item','','plugin').'</a> <a href="?m=zl_admin&c=plugin&a=appcenter&p=1">'.L('plugin_firstpage').'</a> <a href="?m=zl_admin&c=plugin&a=appcenter&p='.$pre_page.'">'.L('plugin_prepage').'</a> <a href="?m=zl_admin&c=plugin&a=appcenter&p='.$next_page.'">'.L('plugin_nextpage').'</a> <a href="?m=zl_admin&c=plugin&a=appcenter&p='.$pages.'">'.L('plugin_lastpage').'</a>';
		$show_header = 1;
