...
class position extends admin {
	...
	public function init() {
			...
			$pages = $this->db->pages;
			$show_dialog = true;
			$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=zl_admin&c=position&a=add\', title:\''.L('posid_add').'\', width:\'500\', height:\'360\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('posid_add'));
	...
	public function delete() {
		...
		$this->_set_cache();
		showmessage(L('posid_del_success'),'?m=zl_admin&c=position');
	}
	...
	public function listorder() {
		if(isset($_POST['dosubmit'])) {
			...
			$this->_set_cache();
			showmessage(L('operation_success'),'?m=zl_admin&c=position');
		} else {
			showmessage(L('operation_failure'),'?m=zl_admin&c=position');
		}
	}
	...
	public function public_item() {	
		if(isset($_POST['dosubmit'])) {
			...
			$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=zl_admin&c=position&a=add\', title:\''.L('posid_add').'\', width:\'500\', height:\'300\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('posid_add'));
