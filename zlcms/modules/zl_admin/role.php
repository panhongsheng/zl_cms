...
class role extends admin {
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			$this->_cache();
			if($insert_id){
				showmessage(L('operation_success'),'?m=zl_admin&c=role&a=init');
			}
		} else {
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			$this->_cache();
			showmessage(L('operation_success'),'?m=zl_admin&c=role');
		} else {					
	...
	public function setting_cat_priv() {
		...
		case 2:
			...
			showmessage(L('operation_success'),'?m=zl_admin&c=role&a=init', '', 'edit');
			break;
