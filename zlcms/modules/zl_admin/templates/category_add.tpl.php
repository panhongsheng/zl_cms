...
<form name="myform" id="myform" action="?m=zl_admin&c=category&a=add" method="post">
<div class="pad-10">
<div class="col-tab">
...
<!--
	...
	function change_tpl(modelid) {
		if(modelid) {
			$.getJSON('?m=zl_admin&c=category&a=public_change_tpl&modelid='+modelid, function(data){$('#template_list').val(data.template_list);$('#category_template').html(data.category_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);});
		}
	}
	function load_file_list(id) {
		if(id=='') return false;
		$.getJSON('?m=zl_admin&c=category&a=public_tpl_file_list&style='+id+'&catid=<?php echo $parentid?>', function(data){$('#category_template').html(data.category_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);});
	}
