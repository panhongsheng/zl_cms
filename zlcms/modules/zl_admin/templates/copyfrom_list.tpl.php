...
?>
<div class="pad-lr-10">
<form name="myform" id="myform" action="?m=zl_admin&c=copyfrom&a=listorder" method="post">
...
function edit(id, name) {
	...
	window.top.art.dialog({title:'<?php echo L('edit');?>《'+name+'》',id:'edit',iframe:'?m=zl_admin&c=copyfrom&a=edit&id='+id,width:'580',height:'240'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;d.document.getElementById('dosubmit').click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
}
function data_delete(obj,id,name){
	window.top.art.dialog({content:name, fixed:true, style:'confirm', id:'data_delete'}, 
	function(){
	$.get('?m=zl_admin&c=copyfrom&a=delete&id='+id+'&pc_hash='+pc_hash,function(data){
				if(data) {
