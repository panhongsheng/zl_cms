...
<form method="post" name="myform" id="myform" action="?m=zl_admin&c=database&a=export">
<input type="hidden" name="tabletype" value="db" id="ZLCMStables">
...
?>   
    ...
    <tbody>
	<?php foreach($infos['ZLCMStables'] as $v){?>
	<tr>
	...
	<td  width="15%" align="center"><a href="?m=zl_admin&c=database&a=public_repair&operation=optimize&pdo_name=<?php echo $pdo_name?>&tables=<?php echo $v['name']?>"><?php echo L('database_optimize')?></a> | <a href="?m=zl_admin&c=database&a=public_repair&operation=repair&pdo_name=<?php echo $pdo_name?>&tables=<?php echo $v['name']?>"><?php echo L('database_repair')?></a> | <a href="javascript:void(0);" onclick="showcreat('<?php echo $v['name']?>','<?php echo $pdo_name?>')"><?php echo L('database_showcreat')?></a></td>
	</tr>
	<?php } ?>
	</tbody>
<?php 
...
<input type="submit" class="button" name="dosubmit" onclick="document.myform.action='?m=zl_admin&c=database&a=public_repair&operation=optimize&pdo_name=<?php echo $pdo_name?>'" value="<?php echo L('batch_optimize')?>"/>
<input type="submit" class="button" name="dosubmit" onclick="document.myform.action='?m=zl_admin&c=database&a=public_repair&operation=repair&pdo_name=<?php echo $pdo_name?>'" value="<?php echo L('batch_repair')?>"/>
</div>
<?php 
}
?>
...
function show_tbl(obj) {
	var pdoname = $(obj).val();
	location.href='?m=zl_admin&c=database&a=export&pdoname='+pdoname+'&pc_hash=<?php echo $_SESSION['pc_hash']?>';
}
function showcreat(tblname, pdo_name) {
	window.top.art.dialog({title:tblname, id:'show', iframe:'?m=zl_admin&c=database&a=public_repair&operation=showcreat&pdo_name='+pdo_name+'&tables=' +tblname,width:'500px',height:'350px'});
}
function reselect() {
