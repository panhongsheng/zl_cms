...
?>   
	<tr>
	<td width="10%">
<input type="checkbox" name="filenames[]" value="<?php echo $info['filename']?>" id="sql_ZLCMS" boxid="sql_ZLCMS">
	</td>
	...
	<a href="javascript:confirmurl('?m=zl_admin&c=database&pdoname=<?php echo $pdoname?>&a=import&pre=<?php echo $info['pre']?>&dosubmit=1', '<?php echo L('confirm_recovery')?>')"><?php echo L('backup_import')?></a>
	</td>
	</tr>
<?php 
	}
...
<input type="submit" class="button" name="dosubmit" value="<?php echo L('backup_del')?>" onclick="document.myform.action='?m=zl_admin&c=database&a=delete&pdoname=<?php echo $pdoname?>';return confirm('<?php echo L('bakup_del_confirm')?>')"/>
</div>
</form>
</div>
</div>
...
function show_tbl(obj) {
	var pdoname = $(obj).val();
	location.href='?m=zl_admin&c=database&a=import&pdoname='+pdoname+'&menuid='+<?php echo $_GET['menuid']?>+'&pc_hash=<?php echo $_SESSION['pc_hash']?>';
}
//-->
</script>