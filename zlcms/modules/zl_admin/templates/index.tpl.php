...
<div class="header">
    ...
    <div class="col-auto">
    	<div class="log white cut_line"><?php echo L('hello'),$admin_username?>  [<?php echo $rolename?>]<span>|</span><a href="?m=zl_admin&c=index&a=public_logout">[<?php echo L('exit')?>]</a><span>|</span>
...
<div id="content">
    ...
    <div class="crumbs">
    <div class="shortcut cu-span"><a href="?m=content&c=create_html&a=public_index&pc_hash=<?php echo $_SESSION['pc_hash'];?>" target="right"><span><?php echo L('create_index')?></span></a><a href="?m=zl_admin&c=cache_all&a=init&pc_hash=<?php echo $_SESSION['pc_hash'];?>" target="right"><span><?php echo L('update_backup')?></span></a><a href="javascript:art.dialog({id:'map',iframe:'?m=zl_admin&c=index&a=public_map', title:'<?php echo L('background_map')?>', width:'700', height:'500', lock:true});void(0);"><span><?php echo L('background_map')?></span></a><?php echo runhook('admin_top_left_menu')?></div>
    <?php echo L('current_position')?><span id="current_pos"></span></div>
    	<div class="col-1">
        	<div class="content" style="position:relative; overflow:hidden">
                <iframe name="right" id="rightMain" src="?m=zl_admin&c=index&a=public_main" frameborder="false" scrolling="auto" style="border:none; margin-bottom:30px" width="100%" height="auto" allowtransparency="true"></iframe>
                <div class="fav-nav">
					<div id="panellist">
...
function site_select(id,name, domain, siteid) {
	...
	$.get("?m=zl_admin&c=index&a=public_set_siteid&siteid="+id,function(data){
		if (data==1){
				window.top.right.location.reload();
				window.top.center_frame.location.reload();
				$.get("?m=zl_admin&c=index&a=public_menu_left&menuid=0&parentid="+$("#bigid").val(), function(data){$('.top_menu').remove();$('#top_menu').prepend(data)});
			}
		});
...
function _M(menuid,targetUrl) {
	...
	$.get("?m=zl_admin&c=index&a=public_current_pos&menuid="+menuid, function(data){
...
function show_help(targetUrl) {
	$("#help").slideUp("slow");
	var str = '';
	$.getJSON("http://v9.help.ZLCMS.cn/api.php?jsoncallback=?",{op:'help',targetUrl: targetUrl},
	function(data){
		if(data!=null) {
			$("#help").slideDown("slow");
...
function session_life() {
	$.get("?m=zl_admin&c=index&a=public_session_life");
}
function lock_screen() {
	$.get("?m=zl_admin&c=index&a=public_lock_screen");
...
function check_screenlock() {
	...
	$.get("?m=zl_admin&c=index&a=public_login_screenlock", { lock_password: lock_password},function(data){
		if(data==1) {
...
function _Site_M(project) {
	...
	$("#leftMain").load("?m=zl_admin&c=index&a=public_menu_left&menuid="+id+'&time='+Math.random());
}
