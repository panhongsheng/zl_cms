...
<form name="myform" action="?m=zl_admin&c=position&a=listorder" method="post">
<div class="pad_10">
...
?>   
	...
	<a href="?m=zl_admin&c=position&a=public_item&posid=<?php echo $info['posid']?>&menuid=<?php echo $_GET['menuid']?>"><?php echo L('posid_item_manage')?></a> |
	...
	<?php } else {?>
	<a href="javascript:confirmurl('?m=zl_admin&c=position&a=delete&posid=<?php echo $info['posid']?>', '<?php echo L('posid_del_cofirm')?>')"><?php echo L('delete')?></a>
	<?php } ?>
	| 
	<?php if($info['thumb']!=""){?>
...
<!--
	...
	window.top.art.dialog({title:'<?php echo L('edit')?>--'+name, id:'edit', iframe:'?m=zl_admin&c=position&a=edit&posid='+id ,width:'500px',height:'360px'}, 	function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;
...
function preview(thumb, name) {
	...
	window.top.art.dialog({title:'预览 '+name+' ',id:'preview',iframe:'?m=zl_admin&c=position&a=preview&thumb='+thumb,width:'530',height:'400'}, '', function(){window.top.art.dialog({id:'preview'}).close()});
}
//-->
</script>