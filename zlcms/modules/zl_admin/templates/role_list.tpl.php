...
<form name="myform" action="?m=zl_admin&c=role&a=listorder" method="post">
...
<td width="5%"><a href="?m=zl_admin&c=role&a=change_status&roleid=<?php echo $info['roleid']?>&disabled=<?php echo ($info['disabled']==1 ? 0 : 1)?>"><?php echo $info['disabled']? L('icon_locked'):L('icon_unlock')?></a></td>
<td  class="text-c">
...
<?php }?>
<a href="?m=zl_admin&c=role&a=member_manage&roleid=<?php echo $info['roleid']?>&menuid=<?php echo $_GET['menuid']?>"><?php echo L('role_member_manage');?></a> |
<?php if($info['roleid'] > 1) {?><a href="?m=zl_admin&c=role&a=edit&roleid=<?php echo $info['roleid']?>&menuid=<?php echo $_GET['menuid']?>"><?php echo L('edit')?></a> |
<a href="javascript:confirmurl('?m=zl_admin&c=role&a=delete&roleid=<?php echo $info['roleid']?>', '<?php echo L('posid_del_cofirm')?>')"><?php echo L('delete')?></a>
<?php } else {?>
...
function setting_role(id, name) {
	...
	window.top.art.dialog({title:'<?php echo L('sys_setting')?>《'+name+'》',id:'edit',iframe:'?m=zl_admin&c=role&a=priv_setting&roleid='+id,width:'700',height:'500'});
}

function setting_cat_priv(id, name) {

	window.top.art.dialog({title:'<?php echo L('usersandmenus')?>《'+name+'》',id:'edit',iframe:'?m=zl_admin&c=role&a=setting_cat_priv&roleid='+id,width:'700',height:'500'});
}
//-->
</script>
</html>
